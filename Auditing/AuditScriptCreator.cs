...
namespace Centeva.Data.Auditing {
	public static class AuditScriptCreator {
		public static IEnumerable<string> GetTriggerScripts(Database db, AuditConfig auditConfig = null, params AuditIgnore[] doNotAudit) {
		...
		private static List<ColumnInfo> GetColumns(Database db, string schema, string table) {
			const string sql = @"
				SELECT DISTINCT
		...
		private static List<TableInfo> GetTables(Database db, string auditTableName, string auditDetailTableName) {
			string sql = $@"
				SELECT SCHEMA_NAME(t.schema_id) AS [Schema], t.name AS [Name], t.modify_date AS [ModDate]
				FROM sys.tables t
				WHERE t.name <> '__MigrationHistory'";

			return db.SqlQuery<TableInfo>(sql).ToList();
		}

		private static List<TriggerInfo> GetExistingTriggers(Database db) {
			const string sql = @"
			...
			return db.SqlQuery<TriggerInfo>(sql).ToList();
		}
